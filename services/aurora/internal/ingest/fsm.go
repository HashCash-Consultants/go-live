...
func (r resumeState) run(s *system) (transition, error) {
	...
	if err != nil {
		return retryResume(r), errors.Wrap(err, "Error getting lastest ledger in hcnet-core")
	}
	...
	if latestLedgerCore < ingestLedger {
		...
		if err != nil {
			return retryResume(r), errors.Wrap(err, "Error fast-forwarding to the latest ledger in hcnet-core")
		}

		if latestLedgerCore == ingestLedger-1 {
			logger.Info("Waiting for ledger to be available in hcnet-core")
		} else {
