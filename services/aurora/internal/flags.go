...
const (
	...
	// HcnetCoreDBURLFlagName is the command line flag for configuring the URL fore Hcnet Core HTTP endpoint
...
func Flags() (*Config, support.ConfigOptions) {
	...
	var flags = support.ConfigOptions{
		...
		&support.ConfigOption{
			...
			FlagDefault: "",
			Required:    false,
			Usage:       "path to hcnet core binary (--remote-captive-core-url has higher precedence)",
			ConfigKey:   &config.HcnetCoreBinaryPath,
		},
		&support.ConfigOption{
		...
		&support.ConfigOption{
			Name:        "hcnet-core-config-path",
			OptType:     types.String,
			FlagDefault: "",
			Required:    false,
			Usage:       "path to hcnet core config file",
			ConfigKey:   &config.HcnetCoreConfigPath,
		},
		&support.ConfigOption{
			...
			FlagDefault: false,
			Required:    false,
			Usage:       "[experimental flag!] causes Aurora to ingest from a Hcnet Core subprocess instead of a persistent Hcnet Core database",
...
func NewAppFromFlags(config *Config, flags support.ConfigOptions) *App {
	...
	if config.HcnetCoreURL == "" {
		log.Fatalf("flag --%s cannot be empty", HcnetCoreURLFlagName)
	}
	if config.Ingest && !config.EnableCaptiveCoreIngestion && config.HcnetCoreDatabaseURL == "" {
		log.Fatalf("flag --%s cannot be empty", HcnetCoreDBURLFlagName)
	}
...
func ApplyFlags(config *Config, flags support.ConfigOptions) {
	...
	if config.EnableCaptiveCoreIngestion {
		binaryPath := viper.GetString("hcnet-core-binary-path")
		remoteURL := viper.GetString("remote-captive-core-url")
		if binaryPath == "" && remoteURL == "" {
			stdLog.Fatalf("Invalid config: captive core requires that either --hcnet-core-binary-path or --remote-captive-core-url is set")
		}
	}
	if config.Ingest {
		// When running live ingestion a config file is required too
		validateBothOrNeither("hcnet-core-binary-path", "hcnet-core-config-path")
	}
