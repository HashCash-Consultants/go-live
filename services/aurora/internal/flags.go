...
func Flags() (*Config, support.ConfigOptions) {
	...
	var flags = support.ConfigOptions{
		...
		&support.ConfigOption{
			...
			OptType:        types.Uint,
			FlagDefault:    uint(0),
			Usage:          "the minimum number of ledgers to maintain within Aurora's history tables (0 = retain an unlimited number of ledgers)",
			UsedInCommands: IngestionCommands,
		},
		&support.ConfigOption{
			...
			Usage:          "the batch size (in ledgers) to remove per reap from the Aurora database",
			UsedInCommands: IngestionCommands,
			CustomSetValue: func(opt *support.ConfigOption) error {
