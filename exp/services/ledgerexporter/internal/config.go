...
func (config *Config) GenerateCaptiveCoreConfig() (ledgerbackend.CaptiveCoreConfig, error) {
	...
	if coreConfig.HcnetCoreBinaryPath == "" {
		var err error
		if coreConfig.HcnetCoreBinaryPath, err = exec.LookPath("hcnet-core"); err != nil {
			return ledgerbackend.CaptiveCoreConfig{}, errors.Wrap(err, "Failed to find hcnet-core binary")
		}
	}
